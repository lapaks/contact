...
while ($row = mysqli_fetch_array($result)) {?>
	...
	<option <?php if ($row['grp_name']==$_POST['grp-name']){ ?> selected="selected"<?php } ?>>
    <?php echo $row['grp_name'];?></option>	
	 <?php } ?>

</select>
	          <button class='btn btn-primary pull-right hidden-xs' type='submit' onclick="prnt_page()"><i class="fa fa-print fa-locate"></i>Print</button>
...
function prnt_page() {
 ...
 <h4><?php echo 'Showing '.$_POST['grp-name'].'';?></h4>
...
if ( $val=='ALL'  ) {
	...
	$sql="SELECT * FROM $tbl_name WHERE grp='$val' and del_flag is NULL";
}
$result=mysqli_query($con,$sql);
